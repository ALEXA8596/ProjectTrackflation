...
const eventFilesDir = path.join(__dirname, "olympic_csv");
...
async function createCombinedPlot(data, eventName) {
  const isFieldEvent = ["shot", "discus", "lj", "hj", "pv"].includes(
    eventName.split("_")[0].toLowerCase()
  );

  const years = data.map((d) => d.year);
  const bestValues = isFieldEvent ? data.map((d) => d.max) : data.map((d) => d.min);
  const meanValues = data.map((d) => d.mean);
  const medianValues = data.map((d) => d.median);

  // Filter out null values and calculate regression lines
  const validBestData = years.map((year, i) => ({ year, value: bestValues[i] })).filter(d => d.value !== null);
  const validMeanData = years.map((year, i) => ({ year, value: meanValues[i] })).filter(d => d.value !== null);
  const validMedianData = years.map((year, i) => ({ year, value: medianValues[i] })).filter(d => d.value !== null);

  const bestReg = validBestData.length > 1 ? calculateLinearRegression(validBestData.map(d => d.year), validBestData.map(d => d.value)) : null;
  const meanReg = validMeanData.length > 1 ? calculateLinearRegression(validMeanData.map(d => d.year), validMeanData.map(d => d.value)) : null;
  const medianReg = validMedianData.length > 1 ? calculateLinearRegression(validMedianData.map(d => d.year), validMedianData.map(d => d.value)) : null;
  ...
  var configuration = {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Best Marks Trend",
          data: bestReg ? years.map((y) => bestReg.predict(y)) : Array(years.length).fill(null),
        ...
        {
          label: "Mean Trend",
          data: meanReg ? years.map((y) => meanReg.predict(y)) : Array(years.length).fill(null),
        ...
        {
          label: "Median Trend",
          data: medianReg ? years.map((y) => medianReg.predict(y)) : Array(years.length).fill(null),
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: `${eventName} Performance Trends`,
          font: { size: 16 },
        },
      },
      scales: {
...
async function generateAllPlots() {
  ...
  for (const file of files) {
    try {
      ...
      const combinedBuffer = await createCombinedPlot(data, eventName);

      // Save all plots
      // Create subdirectories if they don't exist
      const plotTypes = ["boxplots", "best_marks", "means", "medians", "combined"];
      ...
      fs.writeFileSync(
        path.join(outputDir, "combined", `${eventName}.png`),
        combinedBuffer
      );
  ...
  var configuration = {
    ...
    data: {
      ...
      datasets: [
        ...
        {
          label: "Best Marks Trend",
          data: years.map((y) => bestReg.predict(y)),
          borderColor: "rgba(255, 99, 132, 0.5)",
          borderDash: [5, 5],
          fill: false,
          tension: 0,
        ...
        {
          label: "Mean Trend",
          data: years.map((y) => meanReg.predict(y)),
          borderColor: "rgba(54, 162, 235, 0.5)",
          borderDash: [5, 5],
          fill: false,
          tension: 0,
        ...
        {
          label: "Median Trend",
          data: years.map((y) => medianReg.predict(y)),
          borderColor: "rgba(75, 192, 192, 0.5)",
          borderDash: [5, 5],
          fill: false,
          tension: 0,
